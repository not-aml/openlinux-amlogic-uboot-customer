...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"preboot="\
        ...
        "if itest ${upgrade_step} == 1; then  "\
            "defenv; setenv upgrade_step 2; saveenv; run storeargs;"\
        "fi; "\
    ...
    \
   	"storeargs="\
        "setenv bootargs init=/init console=${console} no_console_suspend storage=${store} logo=osd1,loaded,${fb_addr},${outputmode},full hdmimode=${hdmimode} cvbsmode=${cvbsmode} androidboot.firstboot=${firstboot} hdmitx=${cecconfig} val1=${sdioclk} sdhc_debug=0x00000200 rx_clk_phase=${rx_clk_phase}\0"\
    \
	"switch_bootmode="\
...
/*256MBx16bitx2pcs + 128MBx16bitx2pcs*/
//#define CONFIG_M8_DDR_CHANNEL_SET (CONFIG_M8_DDRX2_S30)
/*256MBx16bitx4pcs*/
...
#define CFG_M8_DDR_CLK    768 //696 //768  //792// (636)

//On board DDR capactiy
#define CFG_M8_DDR3_1GB
//#define CFG_M8_DDR3_1GB_32BIT
//#define CFG_M8_DDR3_1P5GB
...
//col size.   2'b01 : A0~A8,      2'b10 : A0~A9  
	...
	#define CONFIG_M8_DDR_ROW_BITS  (14)
        #define CONFIG_M8_DDR3_CHAN1_ROW_SIZE (2)
        #define CONFIG_M8_DDR3_CHAN1_COL_SIZE (2)
        #define CONFIG_M8_DDR_CHAN1_ROW_BITS  (14)
#elif defined(CFG_M8_DDR3_1GB_32BIT)
        //4Gb(X16) x 2pcs
        #define CONFIG_M8_DDR3_ROW_SIZE (3)
        #define CONFIG_M8_DDR3_COL_SIZE (2)
        #define CONFIG_M8_DDR_ROW_BITS  (15)
        #define CONFIG_M8_DDR3_CHAN1_ROW_SIZE (3)
        #define CONFIG_M8_DDR3_CHAN1_COL_SIZE (2)
        #define CONFIG_M8_DDR_CHAN1_ROW_BITS  (15)
#elif defined(CFG_M8_DDR3_1P5GB)
	//4Gb(X16) x 2pcs(channel0) + 2Gb(X16) x 2pcs(channel1)
	#define CONFIG_M8_DDR3_ROW_SIZE (3)
	#define CONFIG_M8_DDR3_COL_SIZE (2)
	#define CONFIG_M8_DDR_ROW_BITS  (15)
	#define CONFIG_M8_DDR3_CHAN1_ROW_SIZE (2)
...
#if defined(CFG_M8_DDR3_1GB) || defined(CFG_M8_DDR3_1GB_32BIT)
	#define PHYS_MEMORY_SIZE     (0x40000000) // 1GB
#elif defined(CFG_M8_DDR3_1P5GB)
	#define PHYS_MEMORY_SIZE     (0x60000000) // 1.5GB
#elif defined(CFG_M8_DDR3_2GB)
